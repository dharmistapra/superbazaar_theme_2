...
const TwoBanner = ({ data }) => {
  ...
  useEffect(() => {
    setTimeout(() => setAnimate(true), 100); // thoda delay for smooth effect
  }, []);
          ...
          data.map((item, index) => (
            ...
            >
              ...
              >
                {item?.title && (
                  <h2
                    className={`text-2xl md:text-3xl font-bold mb-3 ${
                      index === 0 ? "text-white drop-shadow" : "text-gray-900"
                    }`}
                  >
                    {item.title}
                  </h2>
                ...
                <p
                  className={`mb-4 text-sm md:text-base font-medium ${
                ...
                {item?.link && (
                  <Link href={item?.link || "#"} legacyBehavior>
                    <a className="inline-block bg-black hover:bg-gray-800 text-white font-semibold uppercase text-sm px-5 py-2 rounded-lg shadow-md transition-transform duration-300 hover:-translate-y-1">
